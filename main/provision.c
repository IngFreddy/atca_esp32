/*
 * ATCA ATECC508A for ESP32
 * configure.h
 *
 * Created: 3. 7. 2018 13:46:20
 *  Author: Milan Mucka
 */

#include <stdio.h>
#include <stdlib.h>
#include <stddef.h>
#include <stdbool.h>

#include "cryptoauthlib.h"
#include "provision.h"

static const uint8_t g_ecc508_config[] = {
	0x01, 0x23, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //serial, revision, STATIC
	0x00, 0x00, 0x00, 0x00, 0xEE, 0x00, 0x69, 0x00,
	0xC0, 0x00, 0x55, 0x00,							//I2C, OTP Mode 0x55 - permitted 1 -> 0

	0x8F, 0x8F, 0x8F, 0x8F,	0x8F, 0x8F, 0x8F, 0x42, //SLOT config
	0x8F, 0x0F, 0xC2, 0x8F,	0x0F, 0x0F, 0x0F, 0x0F, //2bytes for slot
	0x0F, 0x0F, 0x0F, 0x0F, 0x0F, 0x0F, 0x0F, 0x0F,
	0x0F, 0x0F, 0x0F, 0x0F,	0x0F, 0x0F, 0x9F, 0x8F,

	0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00,	//Counter 0
	0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, //Counter 1
	0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, //Limited Used for ID15
	0xFF, 0xFF, 0xFF, 0xFF,	0xFF, 0xFF, 0xFF, 0xFF,
	0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00,

	0x1E, 0x00, 0x1E, 0x00, 0x1E, 0x00, 0x1C, 0x00, //KEY Config
	0x13, 0x00, 0x5C, 0x00, 0x1C, 0x00, 0x1C, 0x00, //2bytes for key
	0x1C, 0x00, 0x1C, 0x00, 0x1C, 0x00, 0x1C, 0x00,
	0x1C, 0x00, 0x1C, 0x00, 0x1C, 0x00, 0x1C, 0x00
};


ATCA_STATUS write_config()
{
	ATCA_STATUS status;
	bool islocked;

	//g_ecc508_config[16] = addr;	//set address

	atcab_sleep();
	status = atcab_is_locked(LOCK_ZONE_CONFIG, &islocked);
	CHECK_STATUS(status);

	if(islocked){
		printf("Config is locked!\n");
		return status;
	};
	
	status = atcab_write_config_zone(g_ecc508_config);
	CHECK_STATUS(status);
	
	status = atcab_lock_config_zone();
	CHECK_STATUS(status);
	
	printf("CONFIG Complete\r\n");
	
	//status = write_keys();
	//CHECK_STATUS(status);

	return ATCA_SUCCESS;
}

ATCA_STATUS write_keys() 
{
	const uint8_t key0[] = {
		
		0xf0, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x0f,
		
	};

	const uint8_t key1[] = {
		
		0xf1, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x1f,
		
	};

	const uint8_t key2[] = {
		
		0xf2, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x2f,
		
	};
	
	const uint8_t key3[] = {
		
		0xf3, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x11, 0x3f,
		
	};	
	
	ATCA_STATUS status;
	bool islocked;

	atcab_sleep();

	status = atcab_is_locked(LOCK_ZONE_DATA, &islocked);
	CHECK_STATUS(status);

	if(islocked){
		printf("SLOT 0 locked!\n");
	}

	printf("--Write Data 0--\r\n");
	status = atcab_write_bytes_zone(ATCA_ZONE_DATA, 0, 0, (uint8_t*)&key0, 32);
	status = atcab_read_bytes_zone(ATCA_ZONE_DATA, 0, 0, (uint8_t*)&key0, 32);
	CHECK_STATUS(status);
	PRINT_BYTES(key0, 32);

	printf("--Write Data 1--\r\n");
	status = atcab_write_bytes_zone(ATCA_ZONE_DATA, 1, 0, (uint8_t*)&key1, 32);
	CHECK_STATUS(status);

	printf("--Write Data 2--\r\n");
	status = atcab_write_bytes_zone(ATCA_ZONE_DATA, 2, 0, (uint8_t*)&key2, 32);
	CHECK_STATUS(status);

	printf("--Write Data 3--\r\n");
	status = atcab_write_bytes_zone(ATCA_ZONE_DATA, 3, 0, (uint8_t*)&key3, 32);
	CHECK_STATUS(status);
	
	printf("--Write Data 4--\r\n");
	uint8_t pubkey[64];
	status = atcab_genkey(4, pubkey);
	CHECK_STATUS(status);
	
	printf("--Write Data 5--\r\n");
	status = atcab_genkey(5, pubkey);
	CHECK_STATUS(status);

	printf("--Write Data 6--\r\n");
	status = atcab_genkey(6, pubkey);
	CHECK_STATUS(status);

	printf("Write Complete\r\n");
	printf("Locking Data Zone\r\n");
	status = atcab_lock_data_zone();

	return status;
}
